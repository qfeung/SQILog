...
_说明: 这是"Objective-C ReactiveCocoa"(现在称之为"ReactiveObjC")的遗留介绍. 有关使用"Swift"的最新版本, 请参阅[ReactiveCocoa][] 或 [ReactiveSwift][]_

"ReactiveObjC"(官方称为"ReactiveCocoa"或"RAC")是一个启发于[函数式响应编程][]的"Objective-C"框架. 它提供用于**组合和转换"values"流**的API.

如果你已经熟悉"函数式响应编程"或者了解"ReactiveObjC"的基本前提, 查看这个文件夹中的其它文档, 来了解框架概述, 以及有关它在实践中如何运作的更深入的信息.
...
"ReactiveObjC"被疯狂地文档, 有大量的可用来解释什么是RAC以及如何使用它的介绍资料.

如果你想了解更多, 我们推荐了这个资源, 大致按顺序罗列如下:

1. [介绍](#introduction)
2. [什么情况下使用"ReactiveObjC"](#when-to-use-reactiveobjc)
3. [框架概览][]
4. [基本操作][]
5. [头文件](ReactiveObjC/)
6. 以前回答的 [Stack Overflow](https://github.com/ReactiveCocoa/ReactiveCocoa/wiki)
    问题和 [GitHub issues](https://github.com/ReactiveCocoa/ReactiveCocoa/issues?labels=question&state=closed)
7. 这个文件夹的其余部分
8. [基于"iOS"的函数式响应编程](https://leanpub.com/iosfrp/)
    (eBook)
...
"ReactiveObjC"的灵感来源于[函数响应式编程](http://blog.maybeapps.com/post/42894317939/input-and-output). "RAC"提供捕获当前值和未来值的信号(以"RACSignal"为代表),  而不是被替换和改变的可变变量.

通过对信号的链接, 组合和响应, 软件可以以声明式的方式被书写, 不再需要持续观察和更新值的代码.

例如, 一个输入框可以被绑定与最新的时间, 即使它改变, 而不是使用额外的代码来监听时钟, 然后每秒更新输入框. 它的工作原理和"KVO"非常相似, 只不过用"block"代替了重写`-observeValueForKeyPath:ofObject:change:context:`.
...
信号还可以用来导出状态. 通过代替"观察属性和设置其它属性以响应新值", "RAC"使"根据信号和操作表达属性"成为可能:
...
// are equal.
//
// 创建一个单向绑定, 以便"self.password"和"self.passwordConfirmation"
// 相等时"self.createEnabled"将为"true".
//
...
//
// +combineLatest:reduce: 持有一个信号数组, 当数组中的任何一个信号变化的时候, 它会
// 用数组中所有信号的最新的值来执行这个"block", 并且返回一个新的信号, 这个新的
// 信号以"block"的返回值作为新的"values"来发送.
//
...
不仅仅是"KVO", 基于"基于时间变化的values流"信号都可以被构建, 例如, 信号也可以代表按钮点击:
...
[函数式响应编程]: http://en.wikipedia.org/wiki/Functional_reactive_programming
[基本操作]: Documentation/BasicOperators.md
[框架概览]: Documentation/FrameworkOverview.md